...
def eval_for_vis(work_dir):
    ...
    for data_path, mesh_path in tqdm(zip(data_paths, mesh_paths)):
        ...
        g_init = g_init.unsqueeze(0).repeat(10, 1)
...
def eval_for_isaacgym(work_dir):
    ...
    for data in tqdm(dataloader, total=len(dataloader)):
        ...
        pred = model.batch_refine_grasp_sample(xyz, sample_num, g_init=input_refine_palm_gs, data_scale=dataset.scale)
        xyz = xyz.cpu().numpy()
        ...
        for batch_i in range(batch_size):
            ...
            # # debug vis
            # viser_for_grasp.vis_grasp_scene(selected_grasp_Ts, pc=vis_xyz, max_grasp_num=50)
