...
if __name__ == '__main__':
    ...
    while True:
        ...
        for obj_cat in selected_obj_cats:
            valid_instances_in_cat = [i for i in data_dict[obj_cat] if os.path.basename(i) in valid_instances]
            selected_obj_instances.extend(random.sample(valid_instances_in_cat, min(len(valid_instances_in_cat), per_cat_max_num)))
    ...
    # get mean valid grasp ratio
    # @note 0.9426
    selected_ratio = []
    for instance in selected_obj_instances:
        selected_ratio.append(valid_instances_ratio[os.path.basename(instance)])
    print("mean valid grasp ratio:", np.mean(selected_ratio))
