# Info
- Jens Lundell 这个人很坑，它的 [GitHub - jsll/pytorch\_6dof-graspnet](https://github.com/jsll/pytorch_6dof-graspnet/tree/master)、[GitHub - jsll/position-contrained-6dof-graspnet](https://github.com/jsll/position-contrained-6dof-graspnet) 的代码都有问题：
	- pytorch_graspnet：acronym 数据需要自己整合，这个倒不是大问题。大问题是它完全没有定性评估的代码，只有训练。且这份代码和 graspnet 本身一样，只在 6 类物体上训练、测试。
	- contrained graspnet：数据整合的非常好了。但是没有预训练的模型（仓库里提供的还是 pytorch_graspnet 的）。代码不完整，evaluator 部分的代码完全没有。同样没有定性评估的代码。
- 为了能够跑出一个 partial point cloud 条件下、多类别条件下的 graspnet baseline，目前的方案是：以 pytorch_graspnet 的代码为主、用 CONG 的数据集、部分contrained graspnet 的代码（数据集读取工具）、grasp diffusion 中的 isaacgym 仿真评估。

# Bug
- https://github.com/openai/mujoco-py/issues/268 for gl error
- `export LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libGLEW.so` for gl error
